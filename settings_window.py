...
class CostumePreviewPanel(QWidget):
    """Embedded preview panel inside the costume selection page."""
    ...
    def __init__(self, parent=None):
        ...
        self._pending_live2d = None  # (module, path, quality)
        self._update_style()
        ...
        self._load_timer.timeout.connect(self._flush_pending_live2d)

    def _update_style(self):
        dark = isDarkTheme()
        bg = "#1e1e1e" if dark else "#f5f5f5"
        border = BANDORI_PRIMARY_DARK if dark else BANDORI_PRIMARY
        self.setStyleSheet(f"""
            CostumePreviewPanel {{
                background: {bg};
    ...
    def show_live2d(self, live2d_module, model_path: str, quality: str = "balanced"):
        if not model_path:
            self._load_timer.stop()
            self._pending_live2d = None
            self._stack.setCurrentIndex(0)
            return
        # Already showing this model — just make sure it's visible
        if model_path == self._current_model_path and self._live2d_widget is not None:
            self._load_timer.stop()
            self._pending_live2d = None
            self._stack.setCurrentWidget(self._live2d_widget)
            return
        # Queue load; restart debounce timer so only the last hovered model loads
        self._pending_live2d = (live2d_module, model_path, quality)
        self._load_timer.start()

    def _flush_pending_live2d(self):
        pending = self._pending_live2d
        self._pending_live2d = None
        if not pending:
            return
        live2d_module, model_path, quality = pending
        ...
        self._live2d_widget.set_render_quality(quality)
        dark = isDarkTheme()
        r, g, b = (30, 30, 30) if dark else (245, 245, 245)
        self._live2d_widget.set_clear_color(r / 255, g / 255, b / 255, 1.0)
        if model_path != self._current_model_path:
            self._current_model_path = model_path
            self._live2d_widget.set_model_path(model_path)
        self._stack.setCurrentWidget(self._live2d_widget)
...
class SettingsWindow(QWidget):
    ...
    def __init__(self, model_manager, current_char="", current_costume="",
        ...
        _saved_costumes = self._cfg.get("character_costumes", {}) if self._cfg else {}
        if not isinstance(_saved_costumes, dict):
            _saved_costumes = {}
        self._last_costume_per_char: dict[str, str] = {
            **_saved_costumes,
            **{item["character"]: item["costume"] for item in self._configured_models if item.get("character") and item.get("costume")},
        }
    ...
    def _ensure_page(self, key: str) -> QWidget | None:
        ...
        if key == "weather":
            self._weather_page = self._add_lazy_page("weather", self._build_weather_page())
            return self._weather_page
        if key == "memory":
    ...
    def _build_char_page(self):
        ...
        self._switch_char_btn = QPushButton(_tr("SettingsWindow.switch_char"), action_container)
        self._switch_char_btn.setFixedSize(108, 108)
        self._switch_char_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._switch_char_btn.clicked.connect(self._edit_selected_model)
        self._switch_costume_btn = QPushButton(_tr("SettingsWindow.switch_costume"), action_container)
        self._switch_costume_btn.setFixedSize(108, 108)
        self._switch_costume_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._switch_costume_btn.clicked.connect(self._switch_costume_direct)
        switch_row.addStretch()
        switch_row.addWidget(self._switch_char_btn)
        switch_row.addWidget(self._switch_costume_btn)
        switch_row.addStretch()
        action_col.addLayout(switch_row)
    ...
    def _update_switch_button_style(self):
        if not hasattr(self, "_switch_char_btn"):
            return
        dark = isDarkTheme()
        ...
        """
        self._switch_char_btn.setStyleSheet(btn_style)
        self._switch_costume_btn.setStyleSheet(btn_style)
    ...
    def _edit_selected_model(self):
        ...
        self._adding_model = False
        self._char_change_only = True
        self._enter_model_selection()

    def _switch_costume_direct(self):
    ...
    def _build_costume_page(self):
        ...
        self._costume_inline_preview = CostumePreviewPanel(page)
        body_row.addWidget(self._costume_inline_preview, 0, Qt.AlignmentFlag.AlignTop)
    ...
    def _build_weather_page(self):
        ...
        title = TitleLabel(_tr("SettingsWindow.weather_title", "天气"), page)
        layout.addWidget(title)
        subtitle = _wrap_label(SubtitleLabel(_tr(
            "SettingsWindow.weather_subtitle",
            "接入和风天气 API，让角色在对话中感知当前天气与时间。",
        ), page))
        layout.addWidget(subtitle)

        enable_row = QHBoxLayout()
        enable_row.setContentsMargins(0, 0, 0, 0)
        enable_label = BodyLabel(_tr("SettingsWindow.weather_enabled", "启用天气感知"), page)
        self._weather_enabled = SwitchButton(page)
        enable_row.addWidget(enable_label)
        enable_row.addStretch()
        enable_row.addWidget(self._weather_enabled)
        layout.addLayout(enable_row)

        key_label = BodyLabel(_tr("SettingsWindow.weather_private_key", "私钥（PEM 格式，控制台创建凭据时下载）"), page)
        layout.addWidget(key_label)
        self._weather_private_key = QPlainTextEdit(page)
        self._weather_private_key.setPlaceholderText(
            "-----BEGIN PRIVATE KEY-----\nMC4CAQAwBQYDK2VwBCIE...\n-----END PRIVATE KEY-----"
        )
        self._weather_private_key.setFixedHeight(90)
        self._weather_private_key.setFont(QFont("Consolas", 9))
        layout.addWidget(self._weather_private_key)

        id_row = QHBoxLayout()
        id_row.setSpacing(12)
        id_row.setContentsMargins(0, 0, 0, 0)
        kid_col = QVBoxLayout()
        kid_col.setSpacing(4)
        kid_col.addWidget(BodyLabel(_tr("SettingsWindow.weather_key_id", "凭据 ID"), page))
        self._weather_key_id = FluentContextLineEdit(page)
        self._weather_key_id.setPlaceholderText("例：KKB338U2Q2")
        self._weather_key_id.setFixedHeight(36)
        kid_col.addWidget(self._weather_key_id)
        id_row.addLayout(kid_col)
        pid_col = QVBoxLayout()
        pid_col.setSpacing(4)
        pid_col.addWidget(BodyLabel(_tr("SettingsWindow.weather_project_id", "项目 ID"), page))
        self._weather_project_id = FluentContextLineEdit(page)
        self._weather_project_id.setPlaceholderText("例：2ETKDMMW4A")
        self._weather_project_id.setFixedHeight(36)
        pid_col.addWidget(self._weather_project_id)
        id_row.addLayout(pid_col)
        layout.addLayout(id_row)

        host_label = BodyLabel(_tr("SettingsWindow.weather_api_host", "API Host（控制台 → 设置中查看）"), page)
        layout.addWidget(host_label)
        self._weather_api_host = FluentContextLineEdit(page)
        self._weather_api_host.setPlaceholderText("例：abc1234.def.qweatherapi.com")
        self._weather_api_host.setFixedHeight(36)
        layout.addWidget(self._weather_api_host)

        city_label = BodyLabel(_tr("SettingsWindow.weather_city", "位置（LocationID 或 经度,纬度）"), page)
        layout.addWidget(city_label)
        self._weather_city = FluentContextLineEdit(page)
        self._weather_city.setPlaceholderText("例：101240101 或 115.86,28.68")
        self._weather_city.setFixedHeight(36)
        layout.addWidget(self._weather_city)

        layout.addStretch()

        btn_row = QHBoxLayout()
        btn_row.setSpacing(8)
        test_btn = PushButton(FluentIcon.SEARCH, _tr("SettingsWindow.weather_test", "测试连接"), page)
        test_btn.setFixedHeight(36)
        test_btn.clicked.connect(self._test_weather)
        btn_row.addWidget(test_btn)
        save_btn = PrimaryPushButton(FluentIcon.SAVE, _tr("SettingsWindow.llm_save"), page)
        save_btn.setFixedHeight(36)
        save_btn.clicked.connect(self._save_weather_config)
        btn_row.addWidget(save_btn)
        btn_row.addStretch()
        layout.addLayout(btn_row)
    ...
    def _load_weather_config(self):
        if not self._cfg or not hasattr(self, "_weather_enabled"):
            return
        self._weather_enabled.setChecked(bool(self._cfg.get("weather_enabled", False)))
        self._weather_private_key.setPlainText(self._cfg.get("weather_private_key", ""))
        self._weather_key_id.setText(self._cfg.get("weather_key_id", ""))
        self._weather_project_id.setText(self._cfg.get("weather_project_id", ""))
        self._weather_api_host.setText(self._cfg.get("weather_api_host", ""))
        self._weather_city.setText(self._cfg.get("weather_city", ""))

    def _save_weather_config(self):
        if not self._cfg or not hasattr(self, "_weather_enabled"):
            return
        self._cfg.set("weather_enabled", self._weather_enabled.isChecked())
        self._cfg.set("weather_private_key", self._weather_private_key.toPlainText().strip())
        self._cfg.set("weather_key_id", self._weather_key_id.text().strip())
        self._cfg.set("weather_project_id", self._weather_project_id.text().strip())
        self._cfg.set("weather_api_host", self._weather_api_host.text().strip())
        self._cfg.set("weather_city", self._weather_city.text().strip())
        self._cfg.save()
        self.settings_changed.emit({"weather_enabled": self._weather_enabled.isChecked()})
        import weather_manager
        weather_manager.invalidate_cache()
        try:
            from qfluentwidgets import InfoBar, InfoBarPosition
            InfoBar.success(
                title=_tr("SettingsWindow.saved_title", "已保存"),
                content="",
                duration=1500,
                position=InfoBarPosition.TOP,
                parent=self,
    ...
    def _test_weather(self):
        if not hasattr(self, "_weather_private_key"):
            return
        private_key = self._weather_private_key.toPlainText().strip()
        key_id = self._weather_key_id.text().strip()
        project_id = self._weather_project_id.text().strip()
        api_host = self._weather_api_host.text().strip()
        city = self._weather_city.text().strip()
        try:
            import weather_manager
            weather_manager.invalidate_cache()
            ok, detail = weather_manager.test_weather(private_key, api_host, city, key_id, project_id)
        ...
        try:
            ...
            if ok:
                InfoBar.success(
                    title=_tr("SettingsWindow.weather_test_ok", "连接成功"),
                    content=detail,
                    duration=4000,
                    position=InfoBarPosition.TOP,
                    parent=self,
                ...
                InfoBar.error(
                    title=_tr("SettingsWindow.weather_test_fail", "连接失败"),
                    content=detail,
                    duration=6000,
                    position=InfoBarPosition.TOP,
                    parent=self,
    ...
    def _save_configured_models(self):
        ...
        self._cfg.set("character_costumes", dict(self._last_costume_per_char))
        self._cfg.save()
    ...
    def _upsert_configured_model(self, character: str, costume: str):
        ...
        self._last_costume_per_char[character] = costume
        self._selected_list_character = character
        self._editing_list_character = ""
        self._editing_model_index = None
        self._adding_model = False
    ...
    def _on_char_selected(self, char_key: str):
        ...
        if self._char_change_only:
            self._char_change_only = False
            self._selecting_model = False
            costume = (
                self._last_costume_per_char.get(char_key)
                or next(
                    (item["costume"] for item in self._configured_models if item["character"] == char_key),
                    None,
                )
                or self._model_manager.get_default_costume(char_key)
            )
            self._current_costume = costume
            self._selected_costume = costume
            self._upsert_configured_model(char_key, costume)
            self._save_configured_models()
            if char_key and costume:
                self.model_selected.emit(char_key, costume)
            return
        self._selecting_model = True
        self._populate_costumes(char_key)
        display = self._model_manager.get_display_name(char_key)
    ...
    def _populate_costumes(self, char_key: str):
        ...
        if hasattr(self, "_costume_inline_preview"):
            pix = QPixmap()
            img_data = self._model_manager.get_character_image_data(char_key)
            img_path = self._model_manager.get_character_image_path(char_key)
            if img_data:
                pix.loadFromData(img_data)
            elif img_path:
                pix.load(img_path)
            self._costume_inline_preview.set_image(pix if not pix.isNull() else None)
    ...
    def _on_costume_clicked(self, btn: CostumeItem, costume_id: str):
        ...
        self._show_model_detail()
        self._save_configured_models()
        if self._current_char and costume_id:
            self.model_selected.emit(self._current_char, costume_id)
    ...
    def _show_costume_preview(self, anchor: QWidget, costume_id: str):
        if hasattr(self, "_costume_inline_preview"):
            ...
            if live2d_module and model_path:
                self._costume_inline_preview.show_live2d(live2d_module, model_path, self._live2d_quality)
            return
        live2d_module = self._ensure_live2d_preview_module()
    ...
    def connect_ipc_output(self, send_line):
        ...
        self.scale_preview.connect(lambda v: send_line(f"SCALE_PREVIEW\t{v}"))
