...
try:
    from tts_manager import TTSPlayer, TTSRequestWorker, TTSTranslationWorker, strip_tts_action_tags
    _TTS_AVAILABLE = True
...
from shared_constants import _INTERRUPT_COMMANDS
...
class MessageBubble(QWidget):
    ...
    def update_bubble_width(self, viewport_width: int = 0):
        ...
        # Fast path: during streaming, once the bubble is already at max width,
        # skip the expensive natural-width scan and all setFixedWidth calls —
        # only heights change as new tokens arrive.
        if self._streaming and getattr(self, "_last_bubble_target_width", -1) == available_width:
            text_width = max(1, available_width - 24)
            reasoning_text_width = max(1, available_width - 52)
            self._sync_text_label_heights(text_width, reasoning_text_width, True)
        ...
        self._container.updateGeometry()
        self.updateGeometry()
        self._last_bubble_target_width = target_width
    ...
    def set_streaming(self, streaming: bool):
        ...
        self._last_bubble_target_width = -1
        if streaming:
...
class ChatWindow(QWidget):
    ...
    def _init_ui(self):
        ...
        self._input_area_widget = self._build_input_area()
        content_layout.addWidget(self._input_area_widget)
    ...
    def _build_titlebar(self):
        ...
        collapse_btn = IconButton(FluentIcon.CARE_DOWN_SOLID, bar)
        collapse_btn.setFixedSize(32, 32)
        collapse_btn.setToolTip(_tr("ChatWindow.collapse"))
    ...
    def _toggle_collapse(self):
        ...
        if self._collapsed:
            ...
            if self._is_group_chat:
                ...
                self.setMinimumSize(360, 520)
            target_h = max(self._pre_collapse_height, self.minimumHeight())
            ...
            self._collapse_btn.setIcon(FluentIcon.CARE_DOWN_SOLID)
            self._collapse_btn.setToolTip(_tr("ChatWindow.collapse"))
        else:
            # ── Collapse ──────────────────────────────────────────────────────
            self._pre_collapse_height = self.height()
            self._collapsed = True
            self._scroll.hide()
            self._input_area_widget.hide()
            if getattr(self, "_resize_grip", None):
                self._resize_grip.hide()
            self.setMinimumHeight(0)
            start_geo = self.geometry()
            end_geo = QRect(start_geo.x(), start_geo.y(), start_geo.width(), _TITLEBAR_H)
            anim = QPropertyAnimation(self, b"geometry", self)
            anim.setDuration(180)
            anim.setStartValue(start_geo)
            anim.setEndValue(end_geo)
            ...
            anim.start()
            self._collapse_anim = anim
            self._collapse_btn.setIcon(FluentIcon.CARE_UP_SOLID)
            self._collapse_btn.setToolTip(_tr("ChatWindow.expand"))
    ...
    def _apply_theme(self):
        ...
        handle_color = '#4c5569' if dark else '#b0bdd4'
        self._scroll.setStyleSheet(f"""
            ...
            QScrollBar::handle:vertical:hover {{
                background: {'#69758d' if dark else '#8a97b0'};
            }}
            QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical {{
                height: 0px;
            }}
    ...
    def _relayout_message_bubbles(self):
        ...
        # Notify the scroll area to recalculate scrollbar ranges after bubble heights change
    ...
    def _enqueue_tts_text(self, text: str, character: str):
        ...
        while True:
            m = _TTS_SENT_CUT_RE.search(self._tts_text_buffer)
            if m is None:
                break
            sentence = self._tts_text_buffer[:m.end()].strip()
            self._tts_text_buffer = self._tts_text_buffer[m.end():]
            if sentence:
                self._dispatch_tts_sentence(sentence, character)
    ...
    def _flush_tts_text(self, character: str):
        ...
        self._tts_text_buffer = ""
        if text:
            self._dispatch_tts_sentence(text, character)
        else:
            self._start_next_tts_request()

    def _dispatch_tts_sentence(self, text: str, character: str):
    ...
    def _on_tts_player_error(self, error_msg: str):
        import logging
        logging.getLogger(__name__).warning("TTSPlayer error: %s", error_msg)
    ...
    def position_next_to_pet(self, pet_window: QWidget):
        ...
        # 使用 screenAt 获取桌宠实际所在的屏幕，而非固定用主屏
