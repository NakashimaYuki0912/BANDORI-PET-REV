...
_log_path = os.path.join(BASE_DIR, "bandori_error.log")
...
def main():
    ...
    # 跨屏 DPI 稳定：所有进程统一 PassThrough，杜绝 round-trip drift
    ...
    _probe = QLocalSocket()
    _probe.connectToServer(ipc_server_name())
    ...
    def start_ssh_tunnel():
        try:
            ...
            if not _ssh_start():
                print("[ssh_tunnel] failed to start (TTS voice may not work)")
        except Exception as exc:
            print(f"[ssh_tunnel] error: {exc}")

    def stop_ssh_tunnel():
        try:
            from ssh_tunnel import stop as _ssh_stop
            _ssh_stop()
        except Exception:
            pass
    ...
    def _tts_port_open() -> bool:
        import socket as _socket
        try:
            with _socket.create_connection(("127.0.0.1", 9880), timeout=0.5):
                return True
        except OSError:
            return False
    ...
    def start_tts_backend():
        ...
        if _tts_port_open():
            return
        python_exe = os.path.join(TTS_BACKEND_DIR, "build_venv", "python.exe")
        if not os.path.isfile(python_exe):
            print(f"TTS backend python not found: {python_exe}")
            return
        ...
        try:
            ...
            tts_process_ref["process"] = proc
        except OSError as exc:
            print(f"Failed to start TTS backend: {exc}")

    def stop_tts_backend():
        proc = tts_process_ref.get("process")
        if proc is None:
            return
        tts_process_ref["process"] = None
        try:
    ...
    def handle_ipc_line(line: str):
        if line.startswith("ACTION\t") or line.startswith("LIP\t"):
            ...
            parts = line.split("\t", 1)
            start_on_costumes = len(parts) > 1 and parts[1] == "1"
            _launch_settings_from_pet(start_on_costumes=start_on_costumes)
        elif line == "ACTIVATE":
    ...
    def _start_settings_process(show_launch=True, hidden=False, start_on_costumes=False):
        cfg.load()
        process = QProcess(app)
        arguments_list = [
        ...
        if hidden:
            arguments_list.append("--hidden")
        program, arguments = process_program_and_args(BASE_DIR, "settings_process.py", arguments_list)
        process.setProgram(program)
        process.setArguments(arguments)
        process.setProcessChannelMode(QProcess.ProcessChannelMode.SeparateChannels)
    ...
    def launch_settings_process(show_launch=True):
        ...
        if existing is not None and existing.state() != QProcess.ProcessState.NotRunning:
            # Process is alive — just tell it to show its window
    ...
    def prewarm_settings_process():
        ...
        if existing is not None and existing.state() != QProcess.ProcessState.NotRunning:
            return
        _start_settings_process(show_launch=False, hidden=True)
    ...
    app.aboutToQuit.connect(stop_tts_backend)
...
if __name__ == "__main__":
    import datetime
    sys.stderr.write(f"\n--- main.py start {datetime.datetime.now()} ---\n")
    sys.stderr.flush()
    try:
        sys.exit(main())
    except Exception:
