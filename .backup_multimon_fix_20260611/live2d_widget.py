...
class Live2DWidget(QOpenGLWidget):
    ...
    def _refresh_system_scale(self):
        """当 widget 移动到不同 DPI 的屏幕时刷新缩放比，同步
        OpenGL viewport 与 Live2D 模型尺寸，避免可见区域计算偏差。"""
        if not self._initialized_gl:
            return
        new_scale = self.devicePixelRatioF()
        if abs(new_scale - self._system_scale) < 1e-6:
            return
    ...
    def initializeGL(self):
        ...
        self._system_scale = (self.screen() or QGuiApplication.primaryScreen()).devicePixelRatio()
